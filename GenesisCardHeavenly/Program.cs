...
using System.IO;
using GenesisCardHeavenly.Properties;

namespace GenesisCardHeavenly {
    public static class Program {
        ...
        private const int ImageWidth = 660;
        private const int ImageHeight = 998;
        private const int MaxTextWidth = 274;
        private const int TextX = 24;
        private const int IllustTextY = 938;
        private const int TitleTextY = 886;
        private const int IdX = 18;
        private const int IdY = 970;
        private const int IdBossX = 32;
        private const int IdBossY = 980;
        ...
        private static readonly Rectangle RawRect = new Rectangle(11, 38, ImageWidth, ImageHeight);
        ...
        private static readonly Rectangle IllustRect = new Rectangle(14, 913, 316, 72);
        ...
        private static int Main(string[] args) {
            ...
            foreach (XmlElement card in XmlGetNodes(XmlGetRootElement(xml), "card")) {
                ...
                var version = Convert.ToByte(XmlGetNodeText(info, "version"));
                if (version != 4)
                    continue;
            ...
            foreach (var pair in cardDict) {
                var bitmap = RenderImage(pair.Value, PrintSerial);
                if (bitmap == null) {
        ...
        private static Bitmap RenderImage(Card card, bool renderId) {
            ...
            if (layerInfo != null)
                g.DrawImage(layerInfo, title ? TitleRect : IllustRect);
            if (layerFrame != null)
                g.DrawImage(layerFrame, FrameRect);
            if (layerTextIllust != null) {
                if (layerTextIllust.Width > MaxTextWidth * 8)
                    g.DrawImage(layerTextIllust, TextX, IllustTextY, MaxTextWidth, layerTextIllust.Height / 8.0f);
                else
                    g.DrawImage(layerTextIllust, TextX, IllustTextY, layerTextIllust.Width / 8.0f, layerTextIllust.Height / 8.0f);
            }
            if (layerTextTitle != null) {
                if (layerTextTitle.Width > MaxTextWidth * 8)
                    g.DrawImage(layerTextTitle, TextX, TitleTextY, MaxTextWidth, layerTextTitle.Height / 8.0f);
                else
                    g.DrawImage(layerTextTitle, TextX, TitleTextY, layerTextTitle.Width / 8.0f, layerTextTitle.Height / 8.0f);
            }
            if (renderId && card.Volume.Type < 2) {
                ...
                g.DrawImage(idBitmap, boss ? IdBossX : IdX, boss ? IdBossY : IdY, idBitmap.Width / 8.0f, idBitmap.Height / 8.0f);
                idG.Dispose();
                idBitmap.Dispose();
            }
            g.Dispose();
